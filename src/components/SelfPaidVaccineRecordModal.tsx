...
import type { SelfPaidVaccineRecord } from "@/types";
import { formatDate } from "@/utils/dateUtils";

interface SelfPaidVaccineRecordModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (
    recordId: string,
  ...
  record: SelfPaidVaccineRecord | null;
}

export default function SelfPaidVaccineRecordModal({
  isOpen,
  onClose,
  onSubmit,
  record,
}: SelfPaidVaccineRecordModalProps) {
  ...
  useEffect(() => {
    if (record && record.status === "vaccinated") {
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" className="flex-1 btn-success">
              {record.status === "vaccinated" ? "保存修改" : "确认接种"}
            </button>
          </div>
        </form>
      </div>
